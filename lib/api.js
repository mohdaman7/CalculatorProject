...
const API_BASE_URL = process.env.NEXT_PUBLIC_VENDOR_API || "https://vendor.epiccalculator.in/api";
...
class ApiService {
  ...
  loadToken() {
    this.token = safeStorage.getItem('calculator_token');
  }
  ...
  async updateHistoryAddress(pincode, addressData) {
    return this.request('/calculator/history/address', {
      method: 'PUT',
      body: JSON.stringify({ pincode, ...addressData }),
    });
  }
