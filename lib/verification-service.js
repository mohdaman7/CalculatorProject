...
class VerificationService {
  ...
  async isWhitelisted(phoneNumber) {
    try {
      ...
      const SUPER_ADMINS = ['9999999999', '7736904372', '8143831846'];
      if (SUPER_ADMINS.includes(normalized)) return true;
  ...
  isAdmin() {
    ...
    if (user) {
      try {
        ...
        const SUPER_ADMINS = ['9999999999', '7736904372', '8143831846'];
        if (SUPER_ADMINS.includes(phone)) return true;

        // Check isAdmin flag from backend
        if (userData.isAdmin === true) return true;
        
        // Also check isSuperAdmin
  ...
  async isAdminAsync() {
    ...
    // Then try to check whitelist for dynamically granted admin
    if (typeof window === 'undefined') return false;
    const user = localStorage.getItem('user');
    if (user) {
      try {
        ...
        // Check if this phone has admin access in whitelist
        const token = localStorage.getItem('calculator_token');
        if (token) {
          const response = await fetch(`${API_BASE_URL}/verification/check-admin-status`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          if (response.ok) {
            const data = await response.json();
            if (data.isAdmin) {
              // Update localStorage for future sync checks
